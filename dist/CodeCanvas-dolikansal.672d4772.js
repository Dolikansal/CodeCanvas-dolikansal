...
class VotingSystem {
    ...
    initializeProjectVotes() {
        sampleProjects.forEach((project)=>{
            if (!this.votes[project.id]) this.votes[project.id] = {
    ...
    upvoteProject(projectId) {
        if (!this.canUserVote(projectId)) return {
            success: false,
        ...
        if (project) project.upvotes = this.votes[projectId].count;
        // Update the project in sampleProjects array
        const sampleProject = sampleProjects.find((p)=>p.id === projectId);
        if (sampleProject) sampleProject.upvotes = this.votes[projectId].count;
        return {
            success: true,
            newCount: this.votes[projectId].count
        };
    }
    ...
    projectsContainer.innerHTML = sortedProjects.map((project, index)=>{
        ...
        return `
                <div class="project-card ${isTopRanked ? 'top-ranked' : ''}">
                    ...
                    ${project.previewImage ? `<img src="${project.previewImage}" alt="${project.title}" class="project-image" 
                             onerror="this.outerHTML='<div class=\\'project-placeholder\\'>No Preview Available</div>'">` : '<div class="project-placeholder">No Preview Available</div>'}
                    ...
                    <div class="project-meta">
                        ${project.hasDemo ? '<i class="fas fa-external-link-alt meta-icon"></i> Live Demo Available' : '<i class="fas fa-code meta-icon"></i> Code Only'}
                        ${project.hasReadme ? ' \u2022 <i class="fas fa-file-alt meta-icon"></i> README Included' : ' \u2022 <i class="fas fa-exclamation-triangle meta-icon"></i> No README'}
                    </div>
                    ...
                    <div class="upvote-section">
                        ${project.hasDemo && project.demoUrl ? `<a href="${project.demoUrl}" target="_blank" class="btn-primary" style="text-decoration: none; padding: 0.5rem 1rem; font-size: 0.875rem;">
...
function applyCurrentFilters() {
    ...
    // Apply search filter
    if (searchTerm) filtered = filtered.filter((project)=>{
    ...
    if (needsDemo) filtered = filtered.filter((p)=>p.hasDemo);
    if (selectedTag) filtered = filtered.filter((project)=>project.tags.includes(selectedTag));
    return filtered;
}
...
function handleSearch() {
    ...
    if (searchTerm) clearSearchBtn.style.display = 'flex';
...
} else icon.textContent = "\uD83C\uDF19"; // Moon in light mode
...
document.addEventListener("DOMContentLoaded", ()=>{
    ...
    const observer1 = new IntersectionObserver((entries1, observer1)=>{
    ...
    faders.forEach((fade)=>observer1.observe(fade));
});
...
//# sourceMappingURL=CodeCanvas-dolikansal.672d4772.js.map
